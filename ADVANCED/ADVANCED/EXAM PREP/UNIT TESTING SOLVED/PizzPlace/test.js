...
describe("pizzaPlace functionality", () => {
  describe("makeAnOrder ()", () => {
    it("without pizza", () => {
      let onlyDrink = { orderedDrink: "drink" };
      expect(function () {
        pizzUni.makeAnOrder(onlyDrink);
      }).to.throw("You must order at least 1 Pizza to finish the order.");
    });
    it("without drink", () => {
      let onlyPizza = { orderedPizza: "pizza" };
      expect(pizzUni.makeAnOrder(onlyPizza)).to.equal(
        `You just ordered ${onlyPizza.orderedPizza}`
      );
    });
    it("drink and pizza", () => {
      let pizzaDrink = { orderedPizza: "pizza", orderedDrink: "drink" };
      expect(pizzUni.makeAnOrder(pizzaDrink)).to.equal(
        `You just ordered ${pizzaDrink.orderedPizza} and ${pizzaDrink.orderedDrink}.`
      );
    });
  });
  describe("getRemainingWork()", () => {
    ...
    it("equal to zero length", () => {
      expect(pizzUni.getRemainingWork(checkIfEqualZero)).to.equal(
    ...
    it("equal to zero length", () => {
      ...
      expect(pizzUni.getRemainingWork(checkIfAboveZero)).to.equal(
